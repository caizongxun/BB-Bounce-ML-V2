"""
BB反彈ML系統 - Fly.io 优化版本 (V3.1)
支持 Fly.io 永久儲存守拗一次上传模型文檔

修靜显：
1. 榨匹不匹配 Fly.io 編更模形位置 (/mnt/models)
2. 模型文檔在第一次開始時不載入記昄資你希求新昇年
3. 並存檔位置作為 /mnt/models 以永久兄啟動
"""
...
# 斤位更改 MODELS_DIR 签役
# 一一在永久兄嗷中查找模型文檔，不存在則使用上傳的檔案
if (MOUNT_PATH / 'bb_models').exists():
    MODELS_DIR = MOUNT_PATH
    logger.info(f'使用永久存兄模型: {MODELS_DIR}')
else:
    MODELS_DIR = Path('./models')
    logger.warning(f'未找到永久存兄，使用本地檔案: {MODELS_DIR}')
...
if __name__ == '__main__':
    try:
        logger.info('='*60)
        logger.info('BB反彈ML系統 - 實時服務 V3.1 (Fly.io 優化版)
        logger.info('='*60)
        logger.info('模型架構：')
        ...
        logger.info('='*60)
        logger.info(f'檔案路徵: {MODELS_DIR.absolute()}')
