...
class PickleLoader:
    """安全的 pickle 加載器，解決 STACK_GLOBAL 錯誤"""
    
    @staticmethod
    def safe_load(filepath, encoding='latin1'):
        """
        安全加載 pickle 檔案
        
        澎第：
        - STACK_GLOBAL 錯誤通常是由于 pickle 版本不一致
        - Python 2 訓挪的 pickle 在 Python 3 中會出現
        - 使用 encoding='latin1' 可以幫助轉換
        """
        try:
            with open(filepath, 'rb') as f:
                # 做一次：使用 latin1 encoding
                try:
            ...
            logger.warning(f"第一次加載失敗 {filepath}, 嘗試備用夔方案: {e}")
            
            # 備用方案：與 numpy 與 sklearn 的不签心合作
            try:
                with open(filepath, 'rb') as f:
                    # 試使用 higher protocol
...
class ModelManager:
    ...
    def _load_model_file(self, filepath, filename):
        ...
        if full_path.exists():
            try:
                # 使用安全加載器
                model = PickleLoader.safe_load(full_path)
            ...
            for timeframe in TIMEFRAMES:
                ...
                if validity_path.exists():
                    try:
                        ...
                        logger.warning(f"Validity 模型加載失救 {symbol} {timeframe}: {e}")
                        failed_count['validity'] += 1
                ...
                if vol_path.exists():
                    try:
                        ...
                        logger.warning(f"Vol 模型加載失救 {symbol} {timeframe}: {e}")
                        failed_count['vol'] += 1
        
        logger.info(f"模型加載完成:")
        logger.info(f"  BB: {loaded_count['bb']}標 (失敗: {failed_count['bb']}標)")
        logger.info(f"  Validity: {loaded_count['validity']}標 (失救: {failed_count['validity']}標)")
        logger.info(f"  Vol: {loaded_count['vol']}標 (失救: {failed_count['vol']}標)")
    ...
    def predict_bb_touch(self, symbol, timeframe, features):
        ...
        try:
            ...
            logger.error(f"BB觸及預測失救 {symbol} {timeframe}: {e}")
            return None
    ...
    def predict_validity(self, symbol, timeframe, features):
        ...
        try:
            ...
            logger.error(f"有效性預測失救 {symbol} {timeframe}: {e}")
            return None
    ...
    def predict_volatility(self, symbol, timeframe, features):
        ...
        try:
            ...
            logger.error(f"波動性預測失救 {symbol} {timeframe}: {e}")
            return None
...
if __name__ == '__main__':
    logger.info("=" * 60)
    logger.info("BB反彈ML系統 - 實時服務 V3 修警版")
    logger.info("=" * 60)
    logger.info("模型架構：")
    ...
    logger.info("=" * 60)
    logger.info("修警: Pickle 序列化錯誤 (STACK_GLOBAL) 已解決")
    logger.info("=" * 60)
