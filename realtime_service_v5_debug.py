...
class CompleteFeatureExtractor:
    ...
    def calculate_indicators(closes, highs, lows, volumes):
        """計算基本指標"""
        logger.debug(f'[指標計算] 輸入數據: closes={len(closes)}, highs={len(highs)}, lows={len(lows)}, volumes={len(volumes)}')
        ...
        logger.debug(f'[指標計算] RSI={rsi:.2f}, ATR={atr:.2f}, BB_width={bb_width:.2f}')
    ...
    def extract_17_features(symbol, timeframe, prediction_kline):
        ...
        try:
            ...
            if closes is None:
                logger.error(f'[17特徵提取] {symbol}: Binance 返回 None')
                return None
            ...
            logger.debug(f'[17特徵提取] 提取 OHLCV: highs={len(highs)}, lows={len(lows)}, volumes={len(volumes)}')
            ...
            logger.info(f'[17特徵提取] {symbol} {timeframe}: ✅ 成功提取 17 個特徵')
            logger.debug(f'[17特徵提取] 特徵值: {[f"{f:.4f}" for f in features[:5]]}...')
            
            return np.array(features, dtype=np.float32)
        
        except Exception as e:
            logger.error(f'[17特徵提取] {symbol} 失敗: {e}')
...
class ModelLoader:
    ...
    def load_model(filepath):
        ...
        if not filepath.exists():
            logger.warning(f'[模型加載] 文件不存在: {filepath}')
            return None
        try:
...
class ValidityChecker:
        ...
        for symbol in SYMBOLS:
            for timeframe in TIMEFRAMES:
                ...
                if model and scaler:
                    ...
                    logger.warning(f'❌ 加載失敗: {symbol} {timeframe}')
    ...
    def predict(self, symbol, timeframe, features):
        ...
        try:
            logger.debug(f'[有效性預測] {symbol} {timeframe}: 開始預測')
            model = self.models[key]
            scaler = self.scalers[key]
            
            # 標準化特徵
            logger.debug(f'[有效性預測] 特徵形狀: {features.shape}')
            features_scaled = scaler.transform([features])
            logger.debug(f'[有效性預測] 標準化後形狀: {features_scaled.shape}')
            
            proba = model.predict_proba(features_scaled)[0]
            logger.debug(f'[有效性預測] 預測概率: {proba}')
...
class VolatilityPredictor:
    ...
    def predict(self, symbol, timeframe, features):
        ...
        try:
            logger.debug(f'[波動性預測] {symbol} {timeframe}: 開始預測')
            model = self.models[key]
            scaler = self.scalers[key]
            
            # 標準化特徵（提取前15個特徵）
            features_vol = features[:15]
            logger.debug(f'[波動性預測] 特徵形狀: {features_vol.shape}')
            features_scaled = scaler.transform([features_vol])
            predicted_vol = float(model.predict(features_scaled)[0])
            
            logger.debug(f'[波動性預測] 預測波動性: {predicted_vol:.2f}')
...
def health_check():
    return jsonify({
        ...
        'description': 'BB 反彈實時監控系統 V5 (調試版)',
...
def predict():
    ...
    try:
        ...
        # 第二步: 模型預測 (根據 warning_level 觸發模型預測)
        validity_result = None
        volatility_result = None
        
        logger.info(f'[警告等級] {symbol} {timeframe}: {bb_result["warning_level"]}')
        
        # ✅ 修正：根據 warning_level 觸發模型預測
        if bb_result['warning_level'] in ['danger', 'warning', 'caution']:
            ...
            if prediction_kline:
                ...
                if features is not None:
                    logger.info(f'[特徵提取] {symbol}: 成功')
                    logger.debug(f'[特徵] 形狀={features.shape}, 值={features[:3]}')
                    ...
                    logger.info(f'[模型結果] 有效性={validity_result}, 波動性={volatility_result}')
                else:
        ...
        logger.info(f'[回應] {symbol} {timeframe} - 狀態={bb_result["status"]}, 警告={bb_result["warning_level"]}\n')
